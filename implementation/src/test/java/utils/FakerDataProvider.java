...
public class FakerDataProvider {
        ...
        public static Object[][] fullRegisterData() {
            LocalDate birthday = faker.date().birthday(18, 60).toInstant()
                    .atZone(ZoneId.systemDefault())
                    .toLocalDate();
            String day = String.valueOf(birthday.getDayOfMonth());
            String month = birthday.getMonth().name().substring(0, 1).toUpperCase()
                    + birthday.getMonth().name().substring(1).toLowerCase();
            String year = String.valueOf(birthday.getYear());

            return new Object[][] {
                    {
                            faker.name().fullName(),       // full name بدل firstName+lastName
